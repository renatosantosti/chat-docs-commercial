...
export default class ChatDocController
  implements IBaseController<ChatDocRequest, ChatDocResponse>
{
  ...
  public async handler(
  ...
  ): Promise<IBaseHttpResponse<ChatDocResponse | Error>> {
    ...
    try {
      ...
      const response: ChatDocResponse | Error = await this.useCase.handler(
        this.currentUser,
        request,
      );
      ...
      console.error("Occured an error on ChatDocController", {
        error: error.message,
        name: error.name,
      });
