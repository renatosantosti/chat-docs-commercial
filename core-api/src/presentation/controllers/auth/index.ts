...
import { badRequestHttpError, internalHttpError, notFoundHttpError, ok, unauthorizedHttpError,} from "presentation/helpers/http-helper";
...
export default class AuthController implements IBaseNonAuthenticadedController<AuthRequest, AuthResponse> {
  constructor(
    @inject('IAuthUseCase') readonly authUseCase: IAuthUseCase,
  ) {}
  public async handler(request: AuthRequest): Promise<IBaseHttpResponse<AuthResponse | Error>> {
    ...
    try {
      ...
      console.error('Internal Error on auth usecase:', { error: error.message, name: error.name });
