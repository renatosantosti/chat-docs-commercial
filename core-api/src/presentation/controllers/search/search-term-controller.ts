...
@injectable()
export default class SearchTermController
  implements IBaseController<SearchTermRequest, SearchTermResponse>
{
  currentUser?: AuthUserDto;
  constructor(
    @inject("ISearchTermUseCase") readonly useCase: ISearchTermUseCase,
  ) {}

  public async handler(
    currentUser: AuthUserDto,
    request: SearchTermRequest,
  ): Promise<IBaseHttpResponse<SearchTermResponse | Error>> {
    ...
    try {
      ...
      const response: SearchTermResponse | Error = await this.useCase.handler(
        this.currentUser,
        request,
      );
      ...
      console.error("Occured an error on SearchTermController", {
        error: error.message,
        name: error.name,
      });
