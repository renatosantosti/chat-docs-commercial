...
export default class UpdateDocumentController
  implements IBaseController<UpdateDocumentRequest, UpdateDocumentResponse>
{
  currentUser?: AuthUserDto;

  constructor(
    @inject("IUpdateDocumentUseCase") readonly useCase: IUpdateDocumentUseCase,
  ) {}

  public async handler(
    currentUser: AuthUserDto,
    request: UpdateDocumentRequest,
  ): Promise<IBaseHttpResponse<UpdateDocumentResponse | Error>> {
    ...
    try {
      ...
      const requestValidationSchema = Joi.object({
        ...
        userId: Joi.number().greater(0).required().messages({
      ...
      const response: UpdateDocumentResponse | Error =
