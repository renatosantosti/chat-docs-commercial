...
@injectable()
export default class GetAllDocumentController
  implements IBaseController<null, GetAllDocumentResponse>
{
  currentUser?: AuthUserDto;
  constructor(
    @inject("IGetAllDocumentUseCase") readonly useCase: IGetAllDocumentUseCase,
  ) {}

  public async handler(
    currentUser: AuthUserDto,
  ): Promise<IBaseHttpResponse<GetAllDocumentResponse | Error>> {
    this.currentUser = currentUser;
    try {
      // Fires usecase handler
      const response: GetAllDocumentResponse = await this.useCase.handler(
        this.currentUser,
      );
      ...
      console.error("Occured an error to get all documents", {
        error: error.message,
        name: error.name,
      });
