...
export default class DeleteDocumentController implements IBaseController<DeleteDocumentRequest, DeleteDocumentResponse> {
  ...
  constructor(@inject('IDeleteDocumentUseCase') readonly useCase: IDeleteDocumentUseCase) {
  }

  public async handler(currentUser: AuthUserDto, request: DeleteDocumentRequest): Promise<IBaseHttpResponse<DeleteDocumentResponse | Error>> {
    ...
    try {
      ...
      const response: DeleteDocumentResponse | Error = await this.useCase.handler(this.currentUser, request);
      ...
      console.error('Occured an error on DeleteDocumentController:', { error: error.message, name: error.name });
