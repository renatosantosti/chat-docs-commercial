...
export default class CreateDocumentUseCase implements ICreateDocumentUseCase{
  ...
  async handler(currentUser: AuthUserDto, request: CreateDocumentRequest): Promise<CreateDocumentResponse | Error> {
   ...
   this.currentUser = currentUser;
    ...
    try{
      ...
      return new InternalError("An error occurred while try to get pdf from document.");  
    }
    ...
    try {
      ...
      if(!!result){
        ...
        const formatedPages =  newDocument.pages
                                    .map<DocumentTextPageDto>( p => ({
      ...
      return new InternalError("Unknow error occurred while saving new document and try to create search index.");  

    } catch (error: any) {
      console.error('Error creating document:', error);
      return new InternalError("An error occurred while creating the document.");  
    }
  }
}
