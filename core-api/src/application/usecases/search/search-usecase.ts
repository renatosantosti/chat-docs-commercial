...
export default class SearchTermUseCase implements ISearchTermUseCase{
  ...
  async handler(currentUser: AuthUserDto, request: SearchTermRequest): Promise<SearchTermResponse | Error> {
   ...
   this.currentUser = currentUser;
    ...
    if (request.documentId && request.mode == "page" && request.documentId <= 0) {
      return new BadRequestError("Document id is required.");
    }

    if (request.documentId && request.mode == "document") {
      return new BadRequestError("Document is not required when mode is 'document' due all document will be searched.");
    }

   
    let result:SearchResultDto[] = [];
    const filter: SearchTermDto = {
      documentId: request.mode == "page" 
        ? request.documentId 
        : undefined,
      term: request.term,
    }
    ...
    try{
      ...
      const errorMsg =  `Error to search ${request.mode == "document" 
          ? "term on pages from"
