...
export default class UpdateUserUseCase implements IUpdateUserUseCase {
  ...
  async handler(
    ...
    try {
      ...
      const existingUser = await this.repository.getOneById(request.id);
      ...
      const user = {
        ...existingUser,
        id: request.id,
        name: request.name ? request.name : existingUser.name,
        image: request.image ? request.image : existingUser.image,
        password: request.password
          ? await this.hashAdapter.createHash(request.password)
      ...
      console.error("An error occurred while updating the user. Details:", {
        error,
      });
